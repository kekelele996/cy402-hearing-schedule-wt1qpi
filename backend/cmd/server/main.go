...
func main() {
	...
	if err := db.AutoMigrate(
		&model.User{}, &model.Client{}, &model.Case{}, &model.Document{}, &model.Billing{}, &model.Hearing{}, &model.AuditLog{},
	); err != nil {
		logger.Error("auto migrate failed", "error", err.Error())
		os.Exit(1)
	}
	if err := initHearingIndexes(db); err != nil {
		logger.Error("init hearing indexes failed", "error", err.Error())
		os.Exit(1)
	}
	...
	hearingSvc := service.NewHearingService(hearingRepo, caseRepo, userRepo, logger)
